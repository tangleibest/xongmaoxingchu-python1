...
for table_index in range(3, 92):
    ...
    left_up_name = row[2]
    left_up_lng = float(row[3])
    left_up_lat = float(row[4])
    right_up_name = row[5]
    right_up_lng = float(row[6])
    right_up_lat = float(row[7])
    right_lower_name = row[8]
    right_lower_lng = float(row[9])
    right_lower_lat = float(row[10])
    left_lower_name = row[11]
    left_lower_lng = float(row[12])
    left_lower_lat = float(row[13])
    # 计算边长和对角线的长度，单位米
    ...
    if PLATFORM == 'mt':
        ...
        sql = "SELECT client_name,month_sale_num,latitude,longitude,call_center,first_cate_name,second_cate_name,average_price from t_map_client_mt_beijing_mark where " \
              "update_count=%s and own_set_cate not in ('其他品类','超市便利')" % UPDATE_COUNT
        print(business_area_name)

    elif PLATFORM == 'elm':
        # 饿了么
        sql = "SELECT client_name,month_sale_num,latitude,longitude,call_center,first_cate,second_cate from t_map_client_elm_beijing_mark where update_count=%s and own_set_cate not in ('其他品类','超市便利') " % UPDATE_COUNT
...
book.save('%s_beijing_poly_%s.xlsx' % (PLATFORM, UPDATE_COUNT))
