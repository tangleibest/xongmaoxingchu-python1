...
update_sql = "update project set address=%s,latitude=%s,longitude=%s where project_id = %s"
for row in data:
    project_id = row[0]
    project_name = row[1]
    project_list = xm_project_dict.get(project_name, ['', '', '123,423'])
    print(project_list)
    address = project_list[1]
    latitude = project_list[2].split(',')[1]
    longitude = project_list[2].split(',')[0]
    update_list = (address, latitude, longitude, project_id)

    cursor.execute(update_sql, update_list)
db.commit()
