...
sql_me = "SELECT project_id,stall_id from merchant_sales_statistics where stream_date='%s' GROUP BY project_id,stall_id" % yesterday.replace(
    '-', '')

cur_data.execute(sql_me)
results_me = cur_data.fetchall()

dict_me = {}
for row_me in results_me:
    dict_me[str(row_me[0]) + str(row_me[1])] = str(row_me[0]) + str(row_me[1])
...
for row_list in results_income:
    if str(row_list[0]) + str(row_list[5]) in dict_info.keys():
        ...
        if str(project_id2) + str(stall_id2) not in dict_me.keys():
            ...
            list_data[str(row_list[0]) + 'meituan' + stall_name2] = [snowflake_id1, project_id2, project_name3,
                                                                     stall_id2,
                                                                     stall_name2,
                                                                     merchant_id2, merchant_name2, 200, 200, 0, 0,
                                                                     0.0,
                                                                     date, 0, '外卖邦',
                                                                     '外卖邦', '0']
            ...
            list_data[str(row_list[0]) + 'elem' + stall_name2] = [snowflake_id2, project_id2, project_name3,
                                                                  stall_id2,
                                                                  stall_name2,
                                                                  merchant_id2, merchant_name2, 300, 200, 0, 0, 0.0,
                                                                  date,
                                                                  0, '外卖邦',
                                                                  '外卖邦', '0']
...
for insert_row in list_data.values():
    ...
    insert_sql = "INSERT into  merchant_sales_statistics (tid,project_id,project_name,stall_id,stall_name,merchant_id,merchant_name,channel_type,plat_type," \
                 ...
                 % (
                     list2[0], list2[1], list2[2], list2[3], list2[4], list2[5], list2[6], list2[7], list2[8],
                     list2[9],
