...
m_project_info = {"百脑汇": ["北京", "北京市朝阳区朝外大街99号二层", "116.445409,39.923854"],
...
with open('结果.csv', 'w', newline='',
          encoding='utf-8') as csvfile:
    writer = csv.writer(csvfile)
    for project in m_project_info.keys():
        project_value = m_project_info.get(project)
        city = project_value[0]
        address = project_value[1]
        lng = str(project_value[2]).split(",")[0]
        lat = str(project_value[2]).split(",")[1]

        sql = "SELECT client_name,own_set_cate,month_sale_num,average_price,month_sale_num*average_price as income,	ROUND(" \
              "6378.138 * 2 * ASIN(" \
              "SQRT(" \
              "POW(" \
              "SIN(" \
              "(" \
              "latitude * PI() / 180 - %s * PI() / 180" \
              ") / 2" \
              ")," \
              "2" \
              ") + COS(latitude * PI() / 180) * COS(%s * PI() / 180) * POW(" \
              "SIN(" \
              "(" \
              "	longitude * PI() / 180 - %s * PI() / 180" \
              ") / 2" \
              ")," \
              "2" \
              ")" \
              ")" \
              ") * 1000" \
              ") AS distance from t_map_client_mt_beijing_mark where update_count=12  and average_price is not null  HAVING  " \
              "distance<=1000 ORDER BY income desc  LIMIT 50" % (lat, lat, lng)
        cur2 = db2.cursor()
        cur2.execute(sql)
        results2 = cur2.fetchall()
        for row in results2:
            writer.writerow([city, project, row[0], row[1], row[2], row[3], row[4], row[5]])
